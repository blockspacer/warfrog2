...
function FireworkEvent:spawnFirework(pControl)
	...
	x = x + xOffset
	y = y + yOffset
	local z = getTerrainHeight(pControl, x, y)--Return terrain height
...
function FireworkEvent:triggerFireworks(pSob,eventLength, eventCount, finallyLength, finallyCount, range)
	--print(pSob,eventLength, eventCount, finallyLength, finallyCount, range)
	local spawnPlanet = SceneObject(pSob):getZoneName()
	local sobID = SceneObject(pSob):getObjectID()

	writeData(sobID .. ":FireworkEvent:mainEventLength", eventLength)
	writeData(sobID .. ":FireworkEvent:mainEventCount", eventCount)
	writeData(sobID .. ":FireworkEvent:finaleLength", finallyLength)
	writeData(sobID .. ":FireworkEvent:finaleCount", finallyCount)
	writeData(sobID .. ":FireworkEvent:launchRadius", range)
	writeData(sobID .. ":FireworkEvent:heading", math.random(360))
	writeData(sobID .. ":FireworkEvent:spawnPointX", math.floor(SceneObject(pSob):getWorldPositionX()))
	writeData(sobID .. ":FireworkEvent:spawnPointZ", math.floor(SceneObject(pSob):getWorldPositionZ()))
	writeData(sobID .. ":FireworkEvent:spawnPointY", math.floor(SceneObject(pSob):getWorldPositionY()))

	writeStringData(sobID .. ":FireworkEvent:spawnPlanet", spawnPlanet)
	FireworkEvent:startEvent(pSob)
end
