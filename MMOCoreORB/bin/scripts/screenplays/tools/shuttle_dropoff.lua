...
function ShuttleDropoff:spawnShuttle(pPlayer, manualControl)
	...
	local shuttleType = readData(playerID .. ":ShuttleDropoff:shuttleType")
	...
	local posZ = getTerrainHeight(pPlayer, posX, posY)--Return terrain height
	local heading = readData(playerID .. ":ShuttleDropoff:heading")
	local planet = readStringData(playerID .. ":ShuttleDropoff:spawnPlanet")

	if (shuttleType < 3) then
		heading = heading + 180
	end

	local pShuttle = spawnSceneObject(planet, self.shuttleTemplate[shuttleType], posX, posZ, posY, 0, math.rad(heading))
	...
	if (shuttleType ~= 1) then
		CreatureObject(pShuttle):setPosture(PRONE)
	end

	CreatureObject(pShuttle):setCustomObjectName(self.shuttleName[shuttleType])
...
function ShuttleDropoff:triggerFlyby(planet, shuttleType, posX, posY, heading, numSpawns, template, mobLevel)
	local posZ = WFnav:getZ(planet, posX, posY)
	local pController = spawnSceneObject(planet, "object/tangible/theme_park/invisible_object.iff", posX, posZ, posY, 0, heading)
	if (pController == nil) then
		return 0
	end

	local controllerID = SceneObject(pController):getObjectID()

	writeData(controllerID .. ":ShuttleDropoff:shuttleType", shuttleType)
	writeStringData(controllerID .. ":ShuttleDropoff:spawnPlanet", planet)
	writeData(controllerID .. ":ShuttleDropoff:spawnPointX", math.floor(posX))
	writeData(controllerID .. ":ShuttleDropoff:spawnPointY", math.floor(posY))
	writeData(controllerID .. ":ShuttleDropoff:heading", math.floor(heading))
	writeData(controllerID .. ":ShuttleDropoff:numSpawns", numSpawns)

	if (numSpawns > 0) then
		writeStringData(controllerID .. ":ShuttleDropoff:spawnTemplate", template)
		writeData(controllerID .. ":ShuttleDropoff:spawnLevel", mobLevel)
	end

	ShuttleDropoff:doFlyby(pController)
	createEvent(120 * 1000, "ShuttleDropoff", "destroyController", pController, "")
	return pController
end

function ShuttleDropoff:destroyController(pController)
	SceneObject(pController):destroyObjectFromWorld()
end
...
function ShuttleDropoff:doFlyby(pPlayer)
	...
	local shuttleType = readData(SceneObject(pPlayer):getObjectID() .. ":ShuttleDropoff:shuttleType")
	if (SceneObject(pPlayer):isPlayerCreature()) then
		CreatureObject(pPlayer):sendSystemMessage("Beginning flyby.")
	end

	createEvent(6 * 1000, "ShuttleDropoff", "landShuttle", pPlayer, "")
	createEvent((self.shuttleLandingTime[shuttleType] + 9) * 1000, "ShuttleDropoff", "doTakeOff", pPlayer, "")
end
...
function ShuttleDropoff:landShuttle(pPlayer)
	...
	createEvent(self.shuttleLandingTime[shuttleType] * 1000, "ShuttleDropoff", "dropOffNpcs", pPlayer, "")
	if (SceneObject(pPlayer):isPlayerCreature()) then
...
function ShuttleDropoff:dropOffNpcs(pPlayer)
	...
	for i = 1, numSpawns, 1 do
		...
		posY = posY + yOffset
		local posZ = getTerrainHeight(pPlayer, posX, posY)--Return terrain height
		spawnEventMobile(planet, template, mobLevel, posX, posZ, posY, heading, 0)
	end
end
